...
class RadarChart extends React.Component {
  ...
  componentDidMount() {
    ...
    let svg_radar1 = createRadarChart(".radarChart", data, radarChartOptions);
  }
  render() {
    return <div className="radarChart" style={{ display: "inline-flex" }}></div>;
  }
}
...
const createRadarChart = function createRadarChart(parent_selector, data, options) {
  ...
  axis
    ...
    .attr(
      "x",
      (d, i) => rScale(maxValue * cfg.labelFactor) * cos(angleSlice * i - HALF_PI)
    )
    .attr(
      "y",
      (d, i) => rScale(maxValue * cfg.labelFactor) * sin(angleSlice * i - HALF_PI)
    )
    .text((d) => d)
